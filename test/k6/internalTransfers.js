  ...
  for (let i = 0; i < users.length; i += batchSize) {
    ...
    responses.forEach((res, idx) => {
      ...
      if (loginSuccess) {
        ...
        const jwtToken = res.cookies.jwt && res.cookies.jwt[0] ? res.cookies.jwt[0].value : null;
        ...
        if (jwtToken) {
          ...
          console.error(`No JWT cookie found for user: ${batch[idx].account_number}`);
        }
      } else {
        console.error(`Failed to login user: ${batch[idx].account_number}, status: ${res.status}`);
      }
    });
    
    console.log(`Logged in ${Math.min(i + batchSize, users.length)}/${users.length} users`);
  }
...
export default function (sessions) {
  ...
  if (res.status !== 202) {
    console.error(`Transfer failed - Status: ${res.status}, Body: ${res.body}, Account: ${session.account_number}`);
  }
