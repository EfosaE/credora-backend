...
func TestInternalTransfer_Success(t *testing.T) {
	...
	idemp := mocks.NewMockIdempotencyCache(time.Minute)

	svc := operationsvc.NewOperationService(acctRepo, txRepo, idemp, test.SetupTestLogger())
